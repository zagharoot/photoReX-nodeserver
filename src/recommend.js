...
function recommend(req, res, next){
	...
	// read the len of the available recommendation queue. if low, we need to submit a new one to the recommendations queue
	redisClient.llen(qkey, function(err, reply){
		...
		if (len< MIN_QUEUE_LENGTH)		//add a recommendation request more 
		{
